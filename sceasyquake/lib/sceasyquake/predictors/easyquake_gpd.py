...
def _find_easyquake_python() -> str:
    ...
    def _has_tf(python_path: str) -> bool:
        ...
        # Fast file-system check: look for a tensorflow package directory
        # alongside the python binary (avoids spawning a subprocess).
        try:
            ...
            for lib_dir in (
                ...
                if os.path.isdir(os.path.join(lib_dir, 'tensorflow')) or \
                   os.path.isdir(os.path.join(lib_dir, 'tensorflow_core')):
                    return True
            # glob fallback for any python version
            import glob
    ...
    #    Check for TF presence to avoid picking the base env that has
    #    easyQuake installed but not TensorFlow.
    for base in (
    ...
    # 2. Current interpreter — only if it also has TF (e.g. a unified venv)
    if _ilu.find_spec('easyQuake') is not None and _ilu.find_spec('tensorflow') is not None:
        return sys.executable

    # 3. Any conda env candidate that at least exists (TF check may have missed)
    for base in (
        os.path.expanduser('~/anaconda3'),
        os.path.expanduser('~/miniconda3'),
        os.path.expanduser('~/mambaforge'),
